...
class StatusLine(Static):
    ...
    def show_indicator(self, category: str, icon: str, color: str) -> None:
        ...
        Args:
            category: Category name (e.g., "docker", "model", "config")
            icon: Icon to show (e.g., "⋯", "✓", "✗")
            color: Color name or hex (e.g., "cyan", "#6B9E78")
        """
        # For now, use the spinner as a simple indicator
        # TODO: Extend statusline to show multiple contextual indicators
        if icon == "⋯":
    ...
    def hide_indicator(self, category: str) -> None:
        ...
        Args:
            category: Category name to hide
        """
        self.stop_spinner()
...
""")
    ...
    def on_multi_line_input_permission_response(self, event: MultiLineInput.PermissionResponse) -> None:
        ...
        # ========================================================================
        if hasattr(self.session, '_awaiting_permission') and self.session._awaiting_permission:
            print(f"[SimpleTUI] UNIVERSAL permission response received: {event.option}")

            # Get response from option
            response = event.option.get('response')
            print(f"[SimpleTUI] Response value: {response}")

            # Set response on session - PermissionManager is waiting for this
            ...
            try:
                ...
                print(f"[SimpleTUI] Cleared permission buffer")
            except Exception as e:
        ...
        # ========================================================================
        # LEGACY HANDLERS (will be removed once all commands use SDK)
        # ========================================================================
        ...
        # Check if this is workflow permission request
        if hasattr(self.session, '_awaiting_workflow_permission') and self.session._awaiting_workflow_permission:
            try:
                from modules.permission_workflow import WorkflowManager
            except ImportError:
                try:
                    import importlib
                    workflow_mod = importlib.import_module('permission_workflow')
                    WorkflowManager = workflow_mod.WorkflowManager
                except:
                    return
            ...
            # Check if user cancelled
            if event.option.get('response') == PermissionResponse.CANCEL or event.option.get('response') == 'cancel':
                # Cancel the workflow
                if hasattr(self.session, '_workflow_manager') and hasattr(self.session, '_current_workflow_id'):
                    workflow = self.session._workflow_manager.get_workflow(self.session._current_workflow_id)
                    if workflow:
                        workflow.cancel()

                self.session._awaiting_workflow_permission = False
                return

            # User approved - clear waiting flag (workflow will continue)
            self.session._awaiting_workflow_permission = False
            return

        # Check if this is Docker resource selection (NEW UNIFIED FLOW)
        if hasattr(self.session, '_awaiting_docker_resource_selection') and self.session._awaiting_docker_resource_selection:
            try:
                from modules.permission_prompt import PermissionResponse
            except ImportError:
                try:
            ...
            # Check if user cancelled
            if event.option.get('response') == PermissionResponse.CANCEL:
                self.session._awaiting_docker_resource_selection = False
                self.session._docker_resource_choice = None
                return

            # Get resource choice from option data
            self.session._docker_resource_choice = event.option.get('data', {})
            self.session._awaiting_docker_resource_selection = False
            return

        # Check if this is Docker Ollama setup (old flow - resource selection)
        if hasattr(self.session, '_awaiting_docker_ollama_setup') and self.session._awaiting_docker_ollama_setup:
            ...
            # Start unified command execution (permission-first flow)
            import asyncio
            from modules.docker_commands_unified import docker_ollama_setup_unified
            asyncio.create_task(docker_ollama_setup_unified(self, self.session, cpu_limit, memory_limit, gpu_enabled))
            return
        ...
        # Check if this is local check permission request
        if hasattr(self.session, '_awaiting_local_check_permission') and self.session._awaiting_local_check_permission:
            try:
                from modules.permission_prompt import PermissionResponse
            except ImportError:
                try:
            ...
            # Check if user cancelled
            if event.option.get('response') == PermissionResponse.CANCEL:
                self.session._awaiting_local_check_permission = False
                self.write("\n[dim]Local check cancelled[/dim]\n\n")
                # Clear permission prompt
                try:
                    prompt_input = self.query_one("#prompt-input")
                    prompt_input.permission_prompt_data = None
            ...
            # User approved - set flag and re-run command
            self.session._awaiting_local_check_permission = False
            self.session._local_check_approved = True
            ...
            # Re-run the /local command
            import asyncio
            asyncio.create_task(self._handle_user_message('/local', self.query_one("#prompt-input")))
            return

        # Check if this is Ollama setup permission request
        if hasattr(self.session, '_awaiting_ollama_setup_permission') and self.session._awaiting_ollama_setup_permission:
            try:
                from modules.permission_prompt import PermissionResponse
            except ImportError:
                try:
            ...
            # Check if user cancelled
            if event.option.get('response') == PermissionResponse.CANCEL:
                self.session._awaiting_ollama_setup_permission = False
                self.write("\n[dim]Ollama setup cancelled[/dim]\n\n")
                # Clear permission prompt
                try:
                    prompt_input = self.query_one("#prompt-input")
                    prompt_input.permission_prompt_data = None
            ...
            # User approved - set flag and re-run command
            self.session._awaiting_ollama_setup_permission = False
            self.session._ollama_setup_approved = True
            ...
            # Re-run the /docker ollama setup command
            import asyncio
            asyncio.create_task(self._handle_user_message('/docker ollama setup', self.query_one("#prompt-input")))
            return

        # Check if this is providers permission request
        if hasattr(self.session, '_awaiting_providers_permission') and self.session._awaiting_providers_permission:
            self.write("[yellow]━━━ RESPONSE RECEIVED: /model providers permission ━━━[/yellow]\n")
            ...
            # Check if user cancelled
            if event.option.get('response') == PermissionResponse.CANCEL:
                self.write("[dim]User cancelled - clearing permission buffer[/dim]\n")
                self.session._awaiting_providers_permission = False
                self.write("\n[dim]Provider browsing cancelled[/dim]\n\n")
                # Clear permission prompt
                try:
                    prompt_input = self.query_one("#prompt-input")
                    prompt_input.permission_prompt_data = None
            ...
            # User approved - set flag and re-run command
            self.write("[dim]User approved - clearing permission buffer and re-running command[/dim]\n")
            self.session._awaiting_providers_permission = False
            self.session._providers_approved = True
            ...
            # Re-run the /model providers command
            self.write("[dim]Re-running: /model providers[/dim]\n")
            self.write("[yellow]━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━[/yellow]\n\n")
            import asyncio
            asyncio.create_task(self._handle_user_message('/model providers', self.query_one("#prompt-input")))
            return
        ...
        if hasattr(self.session, '_awaiting_provider_selection') and self.session._awaiting_provider_selection:
            ...
            all_models = model_mgr.list_available_models()

            # Filter models for this provider
            models = [m for m in all_models if m.get('provider') == provider_id]
            ...
            models_sorted = sorted(models, key=lambda m: (m.get('ranking', 9999), m.get('name', '')))
    ...
    async def _create_docker_ollama_container(self, cpu_limit: str, memory_limit: str, gpu_enabled: bool) -> None:
        ...
        container_id, container_status, container_name = docker_mgr.get_ollama_container_status()

        if container_id:
            if container_status == 'running':
                self.write(f"[green]✓ Ollama container already running[/green]\n")
                self.write(f"[dim]Container: {container_name}[/dim]\n\n")
                return
            else:
                # Container exists but is not running
                self.write(f"[yellow]⚠ Found existing OpenCLI Ollama container[/yellow]\n")
                self.write(f"[dim]Container: {container_name}[/dim]\n")
                self.write(f"[dim]Status: {container_status}[/dim]\n\n")
                self.write("[cyan]Removing failed container before recreating...[/cyan]\n")

                success, msg = await asyncio.to_thread(
                    docker_mgr.remove_ollama_container,
                    True
                )
                ...
                if success:
                    ...
                    self.write(f"Run [cyan]docker rm -f {container_name}[/cyan] manually, then try again\n\n")
                    return
