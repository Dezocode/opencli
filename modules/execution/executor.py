...
class ExecutionSystem:
    ...
    async def _execute_workflow(
        ...
        try:
            ...
            if step.requires_permission:
                approved = await self._wait_for_step_permission(step, workflow_status, manager, base_prompt)
                if not approved:
                    step.status = StepStatus.CANCELLED
                    workflow_status['steps'][i]['status'] = 'cancelled'
    ...
    async def _wait_for_step_permission(
        ...
        try:
            option = await manager.prompt(self.app, self.session, step_prompt, timeout=30.0)

            if not option or option.get('response') == 'cancel':
                return False

            return option.get('response') == 'allow'
...
_unified_execution_system: Optional[ExecutionSystem] = None
...
def get_executor(app, session) -> ExecutionSystem:
    ...
    if _unified_execution_system is None:
        _unified_execution_system = ExecutionSystem(app, session)
