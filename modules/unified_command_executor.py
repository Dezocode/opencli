...
class UnifiedCommandExecutor:
    ...
    async def execute_command(
        ...
        if permission.requires_approval:
            ...
            # Wait for approval
            while execution.status == "awaiting_permission":
...
class CommandExecution:
    ...
    async def show_initial_permission_prompt(self):
        ...
        self.session._awaiting_command_permission = True
        self.session._current_command_execution = self

    async def execute_with_live_progress(self):
        """Execute command with live progress updates in permission buffer"""
        prompt_input = self.app.query_one("#prompt-input")
        ...
        for i, step in enumerate(self.steps):
            ...
            prompt_input.permission_prompt_data = progress_data
